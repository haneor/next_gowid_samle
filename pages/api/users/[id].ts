...
const userDetailHandler: NextApiHandler = async (request, response) => {
  await new Promise((resolve) => setTimeout(resolve, 500));
  response.json({result: true, data: data.find(d => d.id === request.query.id), message: '유저 목록을 정상 호출하였습니다.' });
}
